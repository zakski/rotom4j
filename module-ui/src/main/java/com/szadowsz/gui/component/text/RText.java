...
public final class RText implements Serializable {
	...
	public void addAttribute(Attribute type, Object value, int beginIdx, int endIdx) {
		...
		if (atrun.size() > 0) {
			...
			while (iter.hasPrevious()) {
				...
				int intersect = action & RTextConstants.I_MODES;
				int combiMode = action & RTextConstants.COMBI_MODES;
				if (combiMode == RTextConstants.MERGE_RUNS) {
					switch (intersect) {
					case RTextConstants.I_TL:
					case RTextConstants.I_CL:
						ar.start = a.start;
						iter.remove();
						break;
					case RTextConstants.I_TR:
					case RTextConstants.I_CR:
						ar.end = a.end;
						iter.remove();
						break;
					}
				} else if (combiMode == RTextConstants.CLIP_RUN) {
					switch (intersect) {
					case RTextConstants.I_CL:
						a.end = ar.start;
						break;
				...
				switch (intersect) {
				case RTextConstants.I_INSIDE:
					iter.remove();
					break;
				case RTextConstants.I_COVERED:
					ar = null;
					break;
				}
			}
	...
	private class AttributeRun implements Serializable {
		...
		private int intersectionWith(AttributeRun ar) {
			...
			if (atype != ar.atype)
				return RTextConstants.I_NONE;
			// Check for combination mode
			int combi_mode = (value.equals(ar.value)) ? RTextConstants.MERGE_RUNS : RTextConstants.CLIP_RUN;
			int sdx = 4, edx = 0;
			// Start index
			if (ar.start < start)
				sdx = 0;
			...
			combi_mode |= RTextConstants.grid[sdx][edx];
			return combi_mode;
		}
