...
//        pg.popMatrix();
        pg.text("TEST", size.x/2, size.y/2);
        graphics.endDraw();
    ...
    protected void updateChildrenCoordinates() { // TODO Fancyify
        ...
        header.updateCoordinates(getParentWindow().getContentStart(),start, new PVector(), headerFittingSize);

        for (RTab tab: getTabs()) {
            tab.updateCoordinates(getParentWindow().getContentStart(),start, new PVector(0,headerFittingSize.y), tabFittingSize);
        }
    }
