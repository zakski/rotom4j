...
public abstract class GValueControl extends GAbstractControl {
	...
	protected int valueType = GConstants.DECIMAL;
	...
	public void setLimits(float start, float end) {
		startLimit = start;
		endLimit = end;
		if (valueType == GConstants.INTEGER) {
			valueType = GConstants.DECIMAL;
			setPrecision(1);
		}
		setEpsilon();
		limitsInvalid = true;
