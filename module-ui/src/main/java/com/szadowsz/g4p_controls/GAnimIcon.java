...
/**
 * 
 * Class for the creation of animated icons to be used with GButton, GLabel,
 * GCheckbox and GOption controls. <br>
 * 
 * All animated icons will have an animation clip called 'ALL' which displays
 * all the frames starting at zero with the user specified frame interval. <br>
 * 
 * Introduced in v4.1
 * 
 * @author Peter Lager
...
public class GAnimIcon extends GIcon {
	 ...
	 * <b>Note:</b> animated icons must only be used with one second control. Use
	 * copy() to get a duplicate icon which can be used with another control.
	 * 
	...
	/**
	 * Animate the icon using store clip details but use the specified frame
	 * interval instead of the stored value. <br>
	...
	/**
	 * Set the current frame to be displayed. The supplied frame number will be
	 * constrained to a valid value. <br>
