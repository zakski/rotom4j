 ...
 * 
 * The button face can have either text or an image or both just pick the right
 * constructor. <br>
 * 
 ...
 * 
 * The image file can either be a single image which is used for all button
 * states, or be a composite of 3 images (tiled horizontally) which are used for
 * the different button states OFF, OVER and DOWN in which case the image width
 * should be divisible by 3. <br>
...
public class GButton extends GTextIconBase {
	 ...
	 * 
	 * This method can be used to change this setting for buttons yet to be created.
	 * Note that it does not affect any existing buttons.
	 * 
	...
	public GButton(PApplet theApplet, float p0, float p1, float p2, float p3, String text) {
		...
		mask.endDraw();
		hotspots = new HotSpot[] { new HotSpot.HSmask(1, mask) // control surface
		};
		...
		createEventHandler(G4P.sketchWindow, "handleButtonEvents", new Class<?>[] { GButton.class, GEvent.class },
	...
	/**
	 * If the parameter is true all 3 event types PRESSED, RELEASED and CLICKED are
	 * generated, if false only CLICKED events are generated (default behaviour).
	 * 
	...
	/**
	 * 
	 * When a mouse button is clicked on a GButton it generates the GEvent.CLICKED
	 * event. If you also want the button to generate GEvent.PRESSED and
	 * GEvent.RELEASED events then you need the following statement.<br>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * is the GButton identifier (variable name) <br>
	 * <br>
	 * 
	 */
	...
	public void draw() {
		if (!visible)
			return;
		updateBuffer(); // Update buffer if invalid
		winApp.push();
