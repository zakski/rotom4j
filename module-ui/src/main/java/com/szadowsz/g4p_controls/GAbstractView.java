...
public abstract class GAbstractView extends GAbstractControl implements PConstants, GConstantsInternal {
	...
	public GAbstractView(PApplet theApplet, float p0, float p1, float p2, float p3) {
		...
		hotspots = new HotSpot[] { new HSrect(1, 0, 0, width, height) // drawing surface
		};
	}

	/**
	 * Get the PGrahics canvas for this view. The returned value should be cast to
	 * Graphics3D if view is 3D (i.e. uses P3D) <br>
	 * 
	 ...
	 * 
	 * @return a PGraphics object showing current state of the view (ignoring
	 *         rotation)
	 */
	public PGraphics getSnapshot() {
		if (view != null) {
			PGraphics snap = winApp.createGraphics(view.width, view.height, PApplet.JAVA2D);
			snap.beginDraw();
			snap.image(view, 0, 0);
			snap.endDraw();
			return snap;
	...
	/**
	 * Converts the event generated by Processing into one relative to the view
	 * control.
	 * 
	...
	protected MouseEvent convertEvent(MouseEvent event, int action, float x, float y) {
		...
		return new MouseEvent(event.getNative(), event.getMillis(), action, event.getModifiers(), mouseX, mouseY,
				button, count);
	}
	...
	/**
	 * Draw the finished view (PGraphics). This method is called by G4P after the
	 * PApplet's draw() method is executed
	 */
	public void draw() {
		if (!visible)
			return;
		if (bufferInvalid && viewListener != null && available) { // Changed to prevent update when unavailable
			viewListener.update();
		}
		winApp.pushStyle();
		winApp.pushMatrix();
	...
	/**
	 * Remove and return the current listener. If there is no listener associated
	 * with this view it returns null.
	 * 
	 * @return the removed listener or null if there was no listener associated with
	 *         this view.
	 */
	public GViewListener removeListener() {
		GViewListener listener = viewListener;
		if (viewListener != null) {
			viewListener.setView(null); // bug fixed ?????? must set view before making null
			viewListener = null; // swapped position with last line
			bufferInvalid = false;
		}
		return listener;
	}
