...
public class GSlider extends GLinearTrackControl {
	...
	public GSlider(PApplet theApplet, float p0, float p1, float p2, float p3, float tr_width) {
		...
		hotspots = new HotSpot[] {
				new HotSpot.HScircle(THUMB_SPOT, width / 2 + (parametricPos - 0.5f) * trackLength, height / 2, trackWidth / 2), // thumb
				new HotSpot.HSrect(TRACK_SPOT, (width - trackLength) / 2, (height - trackWidth) / 2, trackLength, trackWidth), // track
		};
		z = GConstantsInternal.Z_SLIPPY;

		GAbstractControl.epsilon = 0.98f / trackLength;
		...
		registeredMethods = GConstantsInternal.PRE_METHOD | GConstantsInternal.DRAW_METHOD | GConstantsInternal.MOUSE_METHOD;
		cursorOver = HAND;
	...
	public void setEnabled(boolean enable) {
		...
		if (!enable)
			status = GConstantsInternal.OFF_CONTROL;
	}
	...
	protected void updateBuffer() {
		if (bufferInvalid) {
			...
			// Draw thumb
			switch (status) {
			case GConstantsInternal.OFF_CONTROL:
				buffer.fill(palette[TOFF].getRGB());
				break;
			case GConstantsInternal.OVER_CONTROL:
				buffer.fill(palette[TOVER].getRGB());
				break;
			case GConstantsInternal.PRESS_CONTROL:
				buffer.fill(palette[TDOWN].getRGB());
				break;
			case GConstantsInternal.DRAG_CONTROL:
