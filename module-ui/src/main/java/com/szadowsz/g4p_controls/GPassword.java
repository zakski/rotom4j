 ...
 * 
 * Each key typed will display a <b>#</b> character, but the user can specify
 * another character. <br>
 * 
 * Unlike a text field it does not support text selection or the copying and
 * pasting of text via the clipboard. <br>
 * 
...
public final class GPassword extends GAbstractControl implements Focusable {
	...
	public GPassword(PApplet theApplet, float p0, float p1, float p2, float p3, int sbPolicy) {
		...
		createEventHandler(G4P.sketchWindow, "handlePasswordEvents", new Class<?>[] { GPassword.class, GEvent.class },
...
//				setScrollbarValues(ptx, pty);
	...
	protected boolean changeText() {
		StyledString.TextLayoutInfo tli;
	...
	protected void loseFocus(GAbstractControl grabber) {
		...
		if (cursorIsOver == this)
			cursorIsOver = null;
		focusIsWith = grabber;
		// If only blank text clear it out allowing default text (if any) to be
		// displayed
		if (stext.length() > 0) {
