...
/**
 * This class allows you to group GUI controls so that they can be treated as a
 * single entity for common GUI actions. <br>
 * Once grouped a single statement change the enabled status, visibility status,
 * colour scheme and transparency for all the controls in the group. It is
 * possible to specify a delay before the action is executed, and in the case of
 * transparency, specify the amount of time the fade occurs.<br>
 * This is particularly useful if your sketch has a number of 'modes' and each
 * mode has its own set of G4P controls. Simply create a GGroup object for each
 * mode then as the mode changes it is easy to display the controls for the
 * current mode and hide the others. <br>
 * A GGroup is associated with a window (or GWindow) and can only be used with
 * controls displayed on that window.
 *
 * @author Peter Lager
 *
 */
public final class GGroup extends GAbstractControl {
	...
	/**
	 * Create a control group for the indicated PApplet. Only controls created with
	 * this PApplet can be added to this group. <br>
	 * 
	...
	/**
	 * Create a control group for the indicated GWindow. Only controls displayed in
	 * this GWindow can be added to this group. <br>
	 * 
	 * All controls added will be initialised as fully opaque. <br>
	 * 
	...
	/**
	 * Set the transparency of this group of controls and make it unavailable to
	 * mouse and keyboard events if below the threshold. Child controls are ignored?
	 * <br>
	 * 
	...
	/**
	 * Set the transparency of this group of controls and make them unavailable to
	 * mouse and keyboard events if below the threshold. Child controls are ignored?
	 * <br>
	 ...
	 * If you simply want to change the controls' visibility status then use this in
	 * preference to fadeIn/fadeOut
	 * 
	 * @param delay   delay time before action is performed (milli-seconds)
	...
	public void setLocalColorScheme(int delay, int colScheme) {
		...
		actions.add(new Action(COLOR_SCHEME, delay, colScheme)); // new Object[] { colScheme }));
	}
	...
	/*
	 * Loop through all the actions waiting to be processed reducing the time before
	 * they need to be implemented. If the action is ready remove it from the list
	 * and implement it.
	 */
	 ...
	 * If a control is not a valid type for group control, or if the control and
	 * group are for different windows then the control is not added and a warning
	 ...
	 * 
	 * @param controls comma separated list of G4P controls to remove from this
	 *                 group
	 */
