...
/**
 * 
 * This is the base class for all controls that can display text and/or icon
 * i.e. GButton, GLabel, GCheckbox and GOption. <br>
 * 
 * It allows the user to control the position of the icon relative to the text
 * and also their alignment. <br>
 * 
 ...
 * </pre>
 * 
 * to position the icon relative to the text. <br>
 * <br>
 * The icon will reserve just enough space vertically (NORTH/SOUTH) or
 * horizontally (EAST/WEST) to display itself. The <b>setIconAlign(GAlign horz,
 * GAlign vert);</b> can be used to align the icon within its space so usable
 * combinations are <br>
 * <br>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Specifying unusable icon alignments will be accepted but will have no visual
 * effect on the control e.g. <br>
 * 
 * <pre>
 ...
 * <br>
 * 
 * <i>This is new to V4.0.6 replaces the GTextAlign and GTextIconAlignBase
 * classes</i><br>
 * 
 * @author Peter Lager
 * 
...
public abstract class GTextIconBase extends GTextBase {
	...
	/**
	 * 
	 * This will change this label's size so that it just fits round the text and
	 * icon either horizontally, vertically or both. The buffer will also be
	 * resized.
	 * 
	...
	/**
	 * Determines if this control has an icon and/or text then recalculates the size
	 * / position of the text and icon zones,
	 */
	...
	/**
	 * Determines whether this control has an icon then recalculates the size /
	 * position of the text and icon zones. The current text length is zero (i.e. no
	 * text) then a parameter value of true will force the calculation to assume
	 * there is some text. <br>
	 * 
	...
	/**
	 * This is the implementation method for recalculating the size / position of
	 * the text and icon zones,
	 * 
	 * @param hasIcon this control has an icon
	 * @param hasText this control has text to display
	 */
	protected void calcZones(boolean hasIcon, boolean hasText) {
		calcIconZone(hasIcon, hasText); // Calculate icon zone position and size. Also fix alignment of icon in zone
		calcTextZone(hasIcon, hasText); // Calculate text zone
		bufferInvalid = true;
	}
	...
	/**
	 * Set the <b><i>non-animated</i></b> icon to be used, keeping the current icon
	...
	/**
	 * Set the <b><i>non-animated</i></b> icon to be used and the horizontal and/or
	 * vertical icon alignment. Use the constants in GAlign e.g. <b>GAlign.LEFT</b>
	 * <br>
	 * 
	 * You can pass <b>null</b> if you don't want to change a particular alignment.
	 * <br>
	...
	/**
	 * If an animated icon (GAnimIcon) is associated with this control then it is
	 * returned otherwise it returns null. <br>
	 * 
	...
	public GAnimIcon getIcon() {
		return icon instanceof GAnimIcon ? (GAnimIcon) icon : null;
	}
	...
	public void setText(String text, GAlign horz, GAlign vert) {
		if (text == null)
			text = "";
		// If there is no existing text then we need to calculate the size of the text
		// zone
		if (stext.length() == 0)
			calcZones(true);
	...
	private void calcTextZone(boolean hasIcon, boolean hasText) {
	...
	private void calcIconZone(boolean hasIcon, boolean hasText) {
