...
public class GKnob extends GValueControl {
	...
	protected int mode = GConstants.CTRL_HORIZONTAL;
	...
	public GKnob(PApplet theApplet, float p0, float p1, float p2, float p3, float gripAmount) {
		...
		hotspots = new HotSpot[] { new HotSpot.HScircle(1, width / 2, height / 2, gripRadius) };
		z = Z_SLIPPY;
	...
	protected void calculateHotSpot() {
		...
		if (mouseOverArcOnly)
			hotspots[0] = new HotSpot.HSarc(1, width / 2, height / 2, overRad, startAng, endAng); // over grip
		else
			hotspots[0] = new HotSpot.HScircle(1, width / 2, height / 2, overRad);
	}
	...
	public void setTurnMode(int mode) {
		switch (mode) {
		case GConstants.CTRL_ANGULAR:
		case GConstants.CTRL_HORIZONTAL:
		case GConstants.CTRL_VERTICAL:
			this.mode = mode;
		}
	}
