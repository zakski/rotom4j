...
/**
 * This class provides all the key methods needed to respond to mouse events
 * from a GView controlled and render the view. <br>
 * 
 * The user should create their own class that inherits from this class and
 * override the update() method and any of the mouse???() methods they want to
 * use.
 * 
 * @author Peter Lager
 *
...
public class GViewListener implements PConstants {
	...
	/**
	 * Get the PGrahics canvas for this viewer. The returned value should be cast to
	 * Graphics3D is view is 3D (i.e. uses P3D)
	 * 
	...
	/**
	 * Get the platform-native event object. This might be the java.awt event on the
	 * desktop, though if you're using OpenGL on the desktop it'll be a NEWT event
	 * that JOGL uses. Android events are something else altogether. Bottom line,
	 * use this only if you know what you're doing, and don't make assumptions about
	 * the class type.
	 * 
	 ...
	 * You would need to scale this for your mouse by multiplying by the wheel
	 * sensitivity.
	 * 
	 ...
	 * Processing. It creates a new event where the mouse position has been changed
	 * to that relative to the view rather than the sketch window. <br>
	 * Also the action to be used in this event; generally this will be the same as
	 * the original event but the ENTER and EXIT events now relate to the view area
	 * rather than the sketch window. <br>
	 * 
