package com.old.krab.lazy;


import com.old.krab.lazy.input.HotkeySubscriber;
import com.old.krab.lazy.input.InputWatcherBackend;
import com.old.krab.lazy.input.UserInputPublisher;
import com.old.krab.lazy.nodes.AbstractNode;
import com.old.krab.lazy.nodes.NodeType;
import com.old.krab.lazy.nodes.impl.*;
import com.old.krab.lazy.stores.*;
import com.old.krab.lazy.themes.ThemeStore;
import com.old.krab.lazy.utils.ContextLines;
import com.old.krab.lazy.utils.MouseHiding;
import com.old.krab.lazy.utils.SnapToGrid;
import com.old.krab.lazy.windows.Window;
...
import static com.old.krab.lazy.stores.GlobalReferences.gui;
import static com.old.krab.lazy.stores.JsonSaveStore.getGuiDataFolderPath;
import static com.old.krab.lazy.stores.JsonSaveStore.getNextUnusedIntegerFileNameInFolder;
import static com.old.krab.lazy.stores.NodeTree.*;
import static com.old.krab.lazy.stores.NormColorStore.color;
...
public class LazyGui  {
    ...
    public LazyGui(PApplet sketch, LazyGuiSettings settings) {
        ...
        InputWatcherBackend.getInstance();
    ...
    private SliderNode createSliderNode(String path, float defaultValue, float min, float max, boolean constrained) {
        FolderNode folder = findParentFolderLazyInitPath(path);
    ...
    private PlotFolderNode createPlotNode(String fullPath, PVector defaultXY, boolean useZ) {
        FolderNode folder = findParentFolderLazyInitPath(fullPath);
    ...
    public RadioFolderNode radio(String path, String[] options, String defaultOption) {
        ...
        if (node == null) {
            FolderNode parentFolder = findParentFolderLazyInitPath(fullPath);
    ...
    public void colorPickerHueAdd(String path, float hueToAdd) {
        ...
        if (node == null) {
            FolderNode folder = findParentFolderLazyInitPath(fullPath);
            node = new ColorPickerFolderNode(path, folder, color(0,1));
            insertNodeAtItsPath(node);
        } else {
     ...
     *
     * @param path forward slash separated unique path to the control element
     * @param defaultHexColor the same default color for 4 default color stops, useful for homogenous gradients
     * @return PGraphics after endDraw() - ready to be displayed as an image
     */
     ...
     * Faster than get(x,y) by keeping a copy of the gradient in a 1D lookup table in memory.
     *
     * @param path forward slash separated unique path to the control element
    ...
    private void createSavesFolder(){
        insertNodeAtItsPath(new SaveFolderNode(savesFolderName, getRoot()));
    }
