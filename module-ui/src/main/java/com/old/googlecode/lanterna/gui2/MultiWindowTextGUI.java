...
import com.old.googlecode.lanterna.TerminalPosition;
import com.old.googlecode.lanterna.TerminalRectangle;
import com.old.googlecode.lanterna.TerminalSize;
import com.old.googlecode.lanterna.TextColor;
import com.old.googlecode.lanterna.graphics.BasicTextImage;
import com.old.googlecode.lanterna.graphics.TextImage;
import com.old.googlecode.lanterna.gui2.Window.Hint;
import com.old.googlecode.lanterna.input.KeyStroke;
import com.old.googlecode.lanterna.input.KeyType;
import com.old.googlecode.lanterna.input.MouseAction;
import com.old.googlecode.lanterna.screen.Screen;
import com.old.googlecode.lanterna.screen.VirtualScreen;
...
public class MultiWindowTextGUI extends AbstractTextGUI implements WindowBasedTextGUI {
    ...
    public synchronized void updateScreen() throws IOException {
        if (getScreen() instanceof VirtualScreen) {
            ...
            for (Window window : getWindows()) {
                if (window.isVisible()) {
                    if (window.getHints().contains(Hint.FULL_SCREEN) ||
                            window.getHints().contains(Hint.FIT_TERMINAL_WINDOW) ||
                            window.getHints().contains(Hint.EXPANDED)) {
    ...
    protected synchronized void drawGUI(TextGUIGraphics graphics) {
        ...
        for(Window window: getWindows()) {
            if (window.isVisible()) {
                ...
                if (!window.getHints().contains(Hint.NO_DECORATIONS)) {
                ...
                if(!window.getHints().contains(Hint.NO_POST_RENDERING)) {
