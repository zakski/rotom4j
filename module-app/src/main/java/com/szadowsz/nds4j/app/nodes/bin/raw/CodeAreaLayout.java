 ...
 */
public class CodeAreaLayout {

    public CodeAreaLayout() {
    }

    public int computeBytesPerRow(CodeAreaStructure structure, int charactersPerPage) {
    ...
    public int computeCharactersPerRow(CodeAreaStructure structure) {
        CodeAreaViewMode viewMode = structure.getViewMode();
    ...
    public long computeRowsPerDocument(CodeAreaStructure structure) {
        long dataSize = structure.getDataSize();
        int bytesPerRow = structure.getBytesPerRow();
        return dataSize / bytesPerRow + 1;
    }

    public int computePositionByte(CodeAreaStructure structure, int rowCharPosition) {
        CodeType codeType = structure.getCodeType();
        return rowCharPosition / (codeType.getMaxDigitsForByte() + 1);
    }

    public int computeFirstCodeCharacterPos(CodeAreaStructure structure, int byteOffset) {
        CodeType codeType = structure.getCodeType();
        return byteOffset * (codeType.getMaxDigitsForByte() + 1);
    }

    public int computeLastCodeCharPos(CodeAreaStructure structure, int byteOffset) {
        CodeType codeType = structure.getCodeType();
        return byteOffset * (codeType.getMaxDigitsForByte() + 1) + codeType.getMaxDigitsForByte() - 1;
    }

    public CodeAreaCaretPosition computeMovePosition(CodeAreaStructure structure, CodeAreaCaretPosition position, MovementDirection direction, int rowsPerPage) {
        CodeType codeType = structure.getCodeType();
        int bytesPerRow = structure.getBytesPerRow();
        long dataSize = structure.getDataSize();
        CodeAreaSection section = position.getSection().orElse(CodeAreaSection.CODE_MATRIX);
        CodeAreaCaretPosition target = new CodeAreaCaretPosition(position.getDataPosition(), position.getCodeOffset(), section);
        switch (direction) {
            case LEFT: {
