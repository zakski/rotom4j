...
package com.szadowsz.rotom4j.file.nitro.n2d.narc;
...
import com.szadowsz.rotom4j.file.nitro.n2d.narc.data.Fnt;
import com.szadowsz.rotom4j.file.nitro.BaseNFSFile;
import com.szadowsz.rotom4j.file.index.HeaderFile;
...
public class NARC extends BaseNFSFile {
     ...
     * Read NARC data, and create a filename table and a list of files.
     *
     * @param name narc name
     * @param compData binary data
     * @throws NitroException if there are issues reading the data/file
     */
    public NARC(String name, ByteArrayEditableData compData) throws NitroException {
        super(RotomFormat.NARC, name, compData);
        MemBuf buf = MemBuf.create(data);
        MemBuf.MemBufReader reader = buf.reader();
        readGenericNtrHeader(RotomFormat.NARC,reader);
        readFile(reader);
    ...
    public void applyDef(String defPath) throws IOException {
        if (defPath != null) {
            HeaderFile defFile = new HeaderFile(getFileNameWithoutExt(),defPath);
